  ...
  app.get("/api/export/pdf", async (req, res) => {
    try {
      ...
      const htmlContent = `
        ...
        <head>
          <meta charset="UTF-8">
          <title>Relatório de Tempo - System TimeFlow</title>
          <style>
            body {
        ...
        day: 'numeric'
            ...
            ${startDate || endDate ? `<p><strong>Período:</strong> ${startDate ? new Date(startDate as string).toLocaleDateString('pt-BR') : 'Início'
          } até ${endDate ? new Date(endDate as string).toLocaleDateString('pt-BR') : 'Fim'
